...
const UploadCard = (props: any) => {
  ...
  const onSubmit = (data: any) => {
    saveWork();
    // text input is
    let problemStatement = localStorage.getItem("problemstatement-d4sd-prelim-submit");
  ...
  const uploadInput = React.createRef();
    useEffect( ()=> {
      setLoading("plus");
      let problemStatementStored = localStorage.getItem("problemstatement-d4sd-prelim-submit");
      if (problemStatementStored) {
        let text = document.getElementsByClassName("problem-statement-input")[0];
        //@ts-ignore
        ...
        <div className="upload-container">
          {loading != "done" ?
            ...
            <div className="file-btns">
              ...
              <Button className="file-btn" type="primary" onClick={() => {let win = window.open(viewURL, '_blank'); win!.focus();}}>VIEW FILE</Button>
              <Button className="file-btn" type="secondary-outline" onClick={() => {setFileList([]);setLoading("plus");setUploadDisabled(false);}}>REMOVE</Button>
            </div>
            </div>
          }
        </div>
  return (
    ...
    <Form layout={'vertical'} onSubmit={handleSubmit(onSubmit)}>
      ...
      <Row className="bottom-btns">
        <Button className="bottom-btn" type="primary" onClick={() => {saveWork();props.setSubmitStep("projectInfo")}}>BACK</Button>
